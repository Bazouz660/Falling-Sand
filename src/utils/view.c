...
sfBool get_mouse_intersect_view(core_t *c, sfFloatRect to_check)
{
    ...
    sfVector2i pos = {to_check.left, to_check.top};
    sfVector2i pos_offset = {posX_offset, posY_offset};
...
sfVector2i get_mouse_pos_view(core_t *c)
{
    ...
    mouse_pos0, sfRenderWindow_getDefaultView(c->render.window));
    return v2f_to_v2i(mouse_pos);
}
